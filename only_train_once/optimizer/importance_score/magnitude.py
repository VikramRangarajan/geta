...
def importance_score_by_magnitude(param_group):
    ...
    for param, p_transform in zip(param_group["params"], param_group["p_transform"]):
        ...
        if (
            p_transform == TensorTransform.NO_PRUNE
        ):  # consider the case of p_transform is "NO_PRUNE"
            continue
        else:
            if norm_group == None:
...
def importance_score_by_avg_magnitude(param_group):
    ...
    param_group["importance_scores"]["avg_magnitude"] = torch.sqrt(norm_group) / float(
        group_sizes + 1e-6
    )
    ...
    for p_name, param, p_transform in zip(
        ...
        if p_transform == TensorTransform.MULTIHEAD_HEADDIM:
            param_transform = tensor_transformation(
                param, p_transform, param_group["num_groups"], param_group["num_heads"]
            )
        else:
            param_transform = tensor_transformation(
                param, p_transform, param_group["num_groups"]
            )
        if (
            p_transform == TensorTransform.NO_PRUNE
        ):  # consider the case of p_transform is "NO_PRUNE"
