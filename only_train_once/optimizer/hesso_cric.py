...
class HESSOCRIC(BaseHybridSparseOptimizer):
    def __init__(
        ...
        max_cycle_period=10,
        sampling_steps=None,
        hybrid_training_steps=None,
        importance_score_criteria="default",
        proj_per_node_group=True,
    ):

        print("Setup HESSOCRIC")
        self.start_cric_step = start_cric_step
        self.sampling_steps = (
            sampling_steps  # How many sampling step for each cric cycle
        )
        self.max_cycle_period = int(
            max(1, max_cycle_period)
        ...
        for param_group in self.param_groups:
            ...
            if param_group["is_prunable"] and not param_group["is_auxiliary"]:
                ...
                for cycle_period in range(self.max_cycle_period + 1):
                    ...
                    param_group["active_violating_idxes_collection"][cycle_period] = (
                        list()
                    )
        ...
        self.proj_per_node_group = proj_per_node_group

        if self.proj_per_node_group:
            self.start_global_sampling_step = (
                2
                * len(self.trial_group_sparsties)
                * len(self.prunable_param_group_dict)
                + self.start_cric_step
            )
        else:
            self.start_global_sampling_step = self.start_cric_step

        self.is_cric_terminated = False

    def calibrate_param_groups(self):
        self.prunable_param_group_dict = dict()
        for param_group in self.param_groups:
            if param_group["is_prunable"] and not param_group["is_auxiliary"]:
                self.prunable_param_group_dict[param_group["id"]] = param_group
    ...
    def update_violating_set(self, cycle_period=0):
        print("Update violating set, Cycle Period:", cycle_period)
        ...
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                if cycle_period == 1:
                    global_trial_violating_idxes = np.intersect1d(
                        top_indices, group["global_idxes"]
                    )
                    group["active_violating_idxes"] = (
                        global_trial_violating_idxes - group["global_start_idx"]
                    ).tolist()
                else:
                    group["active_violating_idxes"] = [
                        i
                        for i in group["trial_violating_idxes"]
                        if i not in group["historical_violating_idxes"]
                    ]
                group["important_idxes"] = [
                    i
                    for i in range(group["num_groups"])
                    if i not in group["active_violating_idxes"]
                ]
    ...
    def update_trial_violating_set(self):
        ...
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                ...
                group["trial_violating_idxes"] = list(
                    set(group["trial_violating_idxes"])
                )

    def update_historical_violating_set(self):
        for param_group in self.param_groups:
            param_group["historical_violating_idxes"] += param_group[
                "active_violating_idxes"
            ]
            param_group["historical_violating_idxes"] = list(
                set(param_group["historical_violating_idxes"])
            )
    ...
    def proj_trial_group_sparsity(self, param_group, trial_group_sparsity):
        ...
        num_redund_grps = max(
            min(
                int(param_group["num_groups"] * trial_group_sparsity),
                param_group["num_groups"],
            ),
            1,
        )
        _, proj_indices = torch.topk(
            -param_group["importance_scores"]["overall"], num_redund_grps
        )
        proj_raw_indices = proj_indices.cpu().numpy().tolist()
        for p, p_transform in zip(param_group["params"], param_group["p_transform"]):
            proj_indices = index_transformation_param_group(
                proj_raw_indices, p_transform, param_group
            )
    ...
    def cric_step(self):
        ...
        if (
            self.num_steps - self.start_global_sampling_step
        ...
        t = (self.num_steps - self.start_global_sampling_step) % self.sampling_steps
        for group in self.param_groups:
            ...
            if len(group["active_violating_idxes"]) == 0:
                ...
                for p_name, p, p_transform in zip(
                    ...
                    if (
                        ...
                        p.data[:, group["active_violating_idxes"], ...] *= (
                            self.sampling_steps - t - 1.0
                        ) / (self.sampling_steps - t)
                    else:
                        p.data[group["active_violating_idxes"]] *= (
                            self.sampling_steps - t - 1.0
                        ) / (self.sampling_steps - t)
                    ...
                    for ng_id, offset in group["auxiliary_ngs"]:
                        ...
                        for aux_p in aux_pg["params"]:
                            if aux_p.grad is None:
                                continue
                            aux_p.data[offset : offset + group["num_groups"], ...] *= (
                                self.sampling_steps - t - 1.0
                            ) / (self.sampling_steps - t)
    ...
    def proj_step(self, loss):
        ...
        curr_param_group_idx = (self.num_steps - self.start_cric_step) // (
            2 * len(self.trial_group_sparsties)
        )
        curr_trial_group_sparsity_idx = (
            (self.num_steps - self.start_cric_step)
            // 2
            % len(self.trial_group_sparsties)
        )
        do_proj = (self.num_steps - self.start_cric_step) % 2 == 0
        curr_param_group = self.prunable_param_group_dict[
            self.param_group_ids[curr_param_group_idx]
        ]
        curr_trial_group_sparsity = self.trial_group_sparsties[
            curr_trial_group_sparsity_idx
        ]
        ...
        if do_proj:
            ...
            curr_param_group["loss_collection"][self.curr_cycle_period].append(
                loss.item() / self.ref_loss
            )
            self.reset_params()
        curr_param_group["active_violating_idxes_collection"][
            self.curr_cycle_period
        ] = [i for i in range(curr_param_group["num_groups"])]

    def hybrid_step(self):
        t = self.num_steps - self.cric_terminated_step - 1
        for group in self.param_groups:
            if (
                not group["is_prunable"]
                or len(group["active_redundant_idxes"]) == 0
                or self.num_steps
                > self.cric_terminated_step + self.hybrid_training_steps
            ):
                for p_name, p in zip(group["p_names"], group["params"]):
                    if p_name not in group["grad_variant"]:
                        continue
                ...
                for p_name, p, p_transform in zip(
                    ...
                    if (
                        ...
                        p.data[:, active_redundant_idxes, ...] *= (
                            self.hybrid_training_steps - t - 1.0
                        ) / (self.hybrid_training_steps - t)
                    else:
                        p.data[active_redundant_idxes] *= (
                            self.hybrid_training_steps - t - 1.0
                        ) / (self.hybrid_training_steps - t)
    ...
    def step(self, loss=None, closure=None):
        ...
        if self.num_steps < self.start_cric_step:
            self.basic_step()
        elif self.proj_per_node_group and (
            self.num_steps >= self.start_cric_step
            and self.num_steps < self.start_global_sampling_step
        ):
            self.proj_step(loss)
        elif (
            self.num_steps >= self.start_global_sampling_step
            and self.curr_cycle_period < self.max_cycle_period
            and not self.is_cric_terminated
        ):
            if self.num_steps == self.start_global_sampling_step:
                print("Engage into Cycle")
            self.cric_step()
        ...
        for param_group in self.param_groups:
            if param_group["is_prunable"] and not param_group["is_auxiliary"]:
                ...
                for cycle_period in param_group["importance_score_collection"]:
                    importance_score_sample_steps = param_group[
                        "importance_score_collection"
                    ][cycle_period]
                    for importance_score in importance_score_sample_steps:
                        if param_group["accumulated_importance_score"] is None:
                            param_group["accumulated_importance_score"] = (
                                importance_score["overall"].clone()
                            )
                        else:
                            param_group["accumulated_importance_score"] += (
                                importance_score["overall"]
                            )
                        accumulate_count += 1
                if param_group["accumulated_importance_score"] is not None:
                    param_group["accumulated_importance_score"] /= float(
                        accumulate_count
                    )
                ...
                for cycle_period in param_group["active_violating_idxes_collection"]:
                    if (
                        len(
                            param_group["active_violating_idxes_collection"][
                                cycle_period
                            ]
                        )
                        > 0
                    ):
                        violating_idxes = param_group[
                            "active_violating_idxes_collection"
                        ][cycle_period]
                        ...
                        avg_loss_score = (
                            sum(loss_scores) / len(loss_scores) / len(violating_idxes)
                        )
                        param_group["accumulated_importance_score"][
                            violating_idxes
                        ] += self.importance_score_criteria["loss"] * avg_loss_score
    ...
    def identify_redundant_groups(self):
        ...
        _, top_indices = torch.topk(
            -accumulated_global_scores, self.target_num_redundant_groups
        )
    ...
    def commit_important_scores(self, cycle_period):
        for group in self.param_groups:
            if group["is_prunable"] and not group["is_auxiliary"]:
                group["importance_score_collection"][cycle_period].append(
    ...
    def compute_metrics(self):
        ...
        for group in self.param_groups:
            ...
            self.opt_metrics.norm_violating_groups += torch.sum(
                norm_group[violat_idxes]
            ).item()
            ...
            self.opt_metrics.num_trial_violating_groups += len(
                group["trial_violating_idxes"]
            )
            self.opt_metrics.num_historical_violating_groups += len(
                group["historical_violating_idxes"]
            )
