...
def automated_pruning_compression(
    oto_graph,
    model,
    merge_lora_to_base,
    unmerge_lora_to_base,
    export_huggingface_format,
    export_float16,
    full_group_sparse_model_dir,
    compressed_model_dir,
    save_full_group_sparse_model,
    ckpt_format,
):
    ...
    full_group_sparse_model_path = os.path.join(
        full_group_sparse_model_dir, full_group_spase_model_name
    )
    compressed_model_path = os.path.join(compressed_model_dir, compressed_model_name)

    if export_huggingface_format:
        full_group_sparse_model_dir = os.path.join(
            full_group_sparse_model_dir, "huggingface_format_full"
        )
        compressed_model_dir = os.path.join(
            compressed_model_dir, "huggingface_format_compressed"
        )
    ...
    if save_full_group_sparse_model:
        if export_huggingface_format:
            ...
            torch.onnx.export(
                model, oto_graph.dummy_input, full_group_sparse_model_path
            )
    ...
    def find_incoming_node_group_stem_node(
        graph,
        node,
        src_ng,
        visited,
        incoming_node_groups,
        incoming_stem_node_ids,
        verbose=False,
    ):
        if verbose:
    ...
    for node_group in oto_graph.node_groups.values():
        ...
        for node in node_group.nodes.values():
            ...
            if len(incoming_stem_nodes) > 0:
                ...
                incoming_ng = oto_graph.node_groups[
                    incoming_stem_node.node_group_ids[0]
                ]
            ...
            if hasattr(incoming_ng, "op"):
                ...
                head_dim = 1
                if (
                    hasattr(incoming_ng.op, "prune_mode")
                    and incoming_ng.op.prune_mode == "num_head"
                ):
                    ...
                    if num_heads > 1 and head_dim > 1:
                        ...
                        for i in incoming_ng.pruning_redundant_idxes:
                            in_dim_pruned_idxes.extend(
                                [h + i * head_dim for h in range(head_dim)]
                            )
                else:
                    if hasattr(incoming_ng.op, "num_heads"):
                        num_heads = incoming_ng.op.num_heads
            ...
            if node_in.op_name == "flatten" and node.op_name == "linear":
                ...
                for idx in in_dim_pruned_idxes:
                    in_dim_pruned_idxes_refined.extend(
                        [i + idx * expand_time for i in range(expand_time)]
                    )
            ...
            if not node.pruned_status["in_dim"]:
                ...
                node.op.prune_in_dim(
                    pruned_idxes=in_dim_pruned_idxes,
    ...
    if export_huggingface_format:
        ...
        torch.onnx.export(model, oto_graph.dummy_input, compressed_model_path)
