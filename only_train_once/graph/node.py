...
class Node:
    def __init__(
        ...
        super().__init__()
        self.id = id
        # if node belongs to the composed node_group, its op refers to composed op, the leaf_op refers to the basic_op
        self.op = op
        self.leaf_op = op_leaf
        ...
        self.input_shape = []
        self.torch_graph_str = ""
        self.skip_pattern_search = False
    ...
    def title(self):
        ...
        # Default
        title = (
            (self.op_name + "-" + self.op._type)
            if self.op_name != self.op._type
            else self.op._type
        )
    ...
    def is_linear(self):
        return (
            self.op_name == "Linear"
            or self.op_name == "linear"
            or self.op_name == "Gemm"
            or self.op_name == "gemm"
        )

    def is_qlinear(self):
        return self.op_name == "QuantizeLinear"
