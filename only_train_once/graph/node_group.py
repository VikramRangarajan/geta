    ...
    def contain_op(self, op_name):
        for node in self.nodes.values():
            ...
            if (
                ...
                or op_name == node.op_name
                or op_name == node.op._type
            ):
                return True
        return False
...
class NodeGroup(BasicNodeGroup):
    ...
    def get_param_groups(self):
        ...
        for node in self:
            ...
            ng_param_group["op_names"].extend(
                [node_param_groups["op"]] * len(node_param_groups["params"])
            )
            ...
            ng_param_group["node_ids"].extend(
                [node.id for _ in node_param_groups["p_names"]]
            )
    ...
    def set_pruning_redundant_idxes(self):
        ...
        if len(param_groups["params"]) == 0 and not self.is_auxiliary:
            ...
            norm_group = None
            for param, p_transform in zip(
                param_groups["params"], param_groups["p_transform"]
            ):
                ...
                if p_transform == TensorTransform.MULTIHEAD_HEADDIM:
                    param_transform = tensor_transformation(
                        param,
                        p_transform,
                        param_groups["num_groups"],
                        param_groups["num_heads"],
                    )
                    ...
                    for p_transform_type, p_transform_config in p_transform:
                        if (
                            ...
                            param_transform = tensor_transformation(
                                param_transform,
                                p_transform_type,
                                p_transform_config["num_heads"],
                            )
                        elif p_transform_type == TensorTransform.MULTIHEAD_HEADDIM:
                            param_transform = tensor_transformation(
                                param_transform,
                                p_transform_type,
                                p_transform_config["head_dim"],
                                p_transform_config["num_heads"],
                            )
                        else:
                            param_transform = tensor_transformation(
                                param_transform,
                                p_transform_type,
                                num_groups=p_transform_config["num_groups"],
                            )
                else:
                    param_transform = tensor_transformation(
                        param, p_transform, param_groups["num_groups"]
                    )
            ...
            if norm_group is None:
                self.pruning_important_idxes, self.pruning_redundant_idxes = (
                    list(),
                    list(),
                )
                return
            ...
            if hasattr(self, "overwrite_p_transform"):
                if (
                    self.overwrite_p_transform
                    == TensorTransform.MULTIHEAD_NUMHEAD_SPREAD
                    and "head_dim" in param_groups
                ):
                    head_dim = param_groups["head_dim"]
                    head_dim = p_transform_config["head_dim"]
                    refined_pruning_important_idxes = index_transformation(
                        self.pruning_important_idxes,
                        p_transform_type,
                        head_dim=head_dim,
                    )
                    refined_pruning_redundant_idxes = index_transformation(
                        self.pruning_redundant_idxes,
                        p_transform_type,
                        head_dim=head_dim,
                    )
                    self.pruning_important_idxes = np.array(
                        refined_pruning_important_idxes
                    )
                    self.pruning_redundant_idxes = np.array(
                        refined_pruning_redundant_idxes
                    )
                elif isinstance(self.overwrite_p_transform, list):
                    refined_pruning_important_idxes = [
                        i for i in self.pruning_important_idxes
                    ]
                    refined_pruning_redundant_idxes = [
                        i for i in self.pruning_redundant_idxes
                    ]
            ...
            for dependent_node_group in self.dependent_node_groups:
                if len(dependent_node_group.pruning_redundant_idxes) == 0:
                    ...
                    continue
                pruning_redundant_idxes.append(
                    dependent_node_group.pruning_redundant_idxes + offset
                )
                offset += (
                    dependent_node_group.pruning_important_idxes.size
                    + dependent_node_group.pruning_redundant_idxes.size
                )
        ...
        for param, p_transform, node_id in zip(
            param_groups["params"],
            param_groups["p_transform"],
            param_groups["node_ids"],
        ):
            if p_transform == TensorTransform.NO_PRUNE:
                continue
            ...
            if isinstance(p_transform, list):
                ...
                for p_transform_type, p_transform_config in reversed(p_transform):
                    if p_transform_type == TensorTransform.MULTIHEAD_HEADDIM:
                        ...
                        p_transform_type == TensorTransform.MULTIHEAD_NUMHEAD
                        or p_transform_type == TensorTransform.MULTIHEAD_NUMHEAD_SPREAD
                    ):
                        head_dim = p_transform_config["head_dim"]
                        refined_pruning_important_idxes = index_transformation(
                            refined_pruning_important_idxes,
                    ...
                    node.pruning_important_idxes = np.array(
                        refined_pruning_important_idxes
                    )
                    node.pruning_redundant_idxes = np.array(
                        refined_pruning_redundant_idxes
                    )
            else:
    ...
    def prune_out_dim(self, global_skip_modules=set()):
        ...
        for node in self.nodes.values():
            if not node.op:
                continue
            pruning_redundant_idxes = (
                node.pruning_redundant_idxes
                if hasattr(node, "pruning_redundant_idxes")
                else self.pruning_redundant_idxes
            )
            if hasattr(node.op, "prune_out_dim"):
                if (
                    node.op.module not in local_skip_modules
                    and node.op.module not in global_skip_modules
                ):
                    node.op.prune_out_dim(
                        pruned_idxes=pruning_redundant_idxes,
...
class NodeGroupComposedOp(BasicNodeGroup):
    ...
    def get_param_groups(self):
        ...
        basic_attrs = ["op_names", "p_names", "params", "p_transform"]
        ng_param_group["op_names"].extend(
            [op_param_group["op"]] * len(op_param_group["params"])
        )
    ...
    def set_pruning_redundant_idxes(self):
        ...
        if len(param_groups["params"]) == 0 and not self.is_auxiliary:
                ...
                if p_transform == TensorTransform.MULTIHEAD_HEADDIM:
                    ...
                    for p_transform_type, p_transform_config in p_transform:
                        if p_transform_type == TensorTransform.MULTIHEAD_HEADDIM:
                            ...
                            num_heads = p_transform_config["num_heads"]
                            param_transform = tensor_transformation(
                                param_transform, p_transform_type, num_groups=num_heads
                            )
                else:
                    param_transform = tensor_transformation(
                        param, p_transform, param_groups["num_groups"]
            ...
            if self.num_groups == 1:
                ...
                self.pruning_important_idxes = np.arange(self.num_groups)[
                    norm_group != 0
                ]
                self.pruning_redundant_idxes = np.arange(self.num_groups)[
                    norm_group == 0
                ]

    def prune_out_dim(self, **kwargs):
        if hasattr(self.op, "prune_out_dim"):
            self.op.prune_out_dim(
                pruned_idxes=self.pruning_redundant_idxes, skip_output_node=True
            )
            for node in self:
