...
class OTO:
    def __init__(
        ...
        if self._model is not None and self._dummy_input is not None:
            self.initialize(
                ...
                strict_out_nodes=self._strict_out_nodes,
            )
            if self._mode == "prune":
    ...
    def initialize(
        self, model=None, dummy_input=None, skip_patterns=None, strict_out_nodes=False
    ):
        self._model = model
        self._dummy_input = dummy_input
        self._graph = Graph(
    ...
    def visualize(
        ...
        self._graph.build_dot(
            vertical=vertical,
            by_node_groups=by_node_groups,
            display_params=display_params,
            display_flops=display_flops,
            display_macs=display_macs,
        ).render(
            os.path.join(
                out_dir if out_dir is not None else "./",
                self._model.name
                if hasattr(self._model, "name")
                else type(self._model).__name__ + "_zig.gv",
            ),
            view=view,
        )
    ...
    # Add the variant of hesso optimizer to handle the QAT case
    def geta(
        self,
        lr=0.1,
        lr_quant=1e-3,
    ...
    #     fixed_zero_groups=True,
    #     lora_update_freq=4,
    #     importance_score_criteria=None,
    # ):
    ...
    #         importance_score_criteria=importance_score_criteria,
    #         proj_per_node_group=proj_per_node_group,
    #     )
    #     return self._optimizer
    ...
    def construct_subnet(
        self,
        merge_lora_to_base=False,
        unmerge_lora_to_base=False,
        export_huggingface_format=False,
        export_float16=False,
        out_dir="./",
        full_group_sparse_model_dir=None,
        compressed_model_dir=None,
        save_full_group_sparse_model=True,
        ckpt_format="torch",
    ):
        full_group_sparse_model_dir = (
            out_dir
            if full_group_sparse_model_dir is None
            else full_group_sparse_model_dir
        )
        compressed_model_dir = (
            out_dir if compressed_model_dir is None else compressed_model_dir
        )
        if self._mode == "prune":
            self.compressed_model_path, self.full_group_sparse_model_path = (
    ...
    def compute_macs(self, in_million=True, in_billion=False, layerwise=False):
        return self._graph.compute_macs(
            in_million=in_million, in_billion=in_billion, layerwise=layerwise
        )

    def compute_bops(self, in_million=True, in_billion=False, layerwise=False):
        return self._graph.compute_bops(
            in_million=in_million, in_billion=in_billion, layerwise=layerwise
        )

    def compute_num_params(self, in_million=True, in_billion=False):
        return self._graph.compute_num_params(
            in_million=in_million, in_billion=in_billion
        )

    def compute_weight_size(self, in_million=True, in_billion=False):
        return self._graph.compute_weight_size(
            in_million=in_million, in_billion=in_billion
        )

    def compute_average_bit_width(self):
        return float(self._graph.compute_weight_size()["total"]) / float(
            self.compute_num_params()
        )

    def print_layer_breakdown(self, macs_info, bops_info):
        self._graph.print_layer_breakdown(macs_info, bops_info)
