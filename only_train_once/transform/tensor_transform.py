...
def tensor_transformation_param_group(tensor, transformation_type, param_group):
    if transformation_type == TensorTransform.MULTIHEAD_HEADDIM:
        return tensor_transformation(
            tensor,
            transformation_type,
        ...
        transformation_type == TensorTransform.MULTIHEAD_NUMHEAD
        or transformation_type == TensorTransform.MULTIHEAD_NUMHEAD_SPREAD
    ):
        return tensor_transformation(
            tensor, transformation_type, param_group["num_groups"]
        )
    elif isinstance(transformation_type, list):
        tensor_transformed = tensor.clone()
        for p_transform_type, p_transform_config in transformation_type:
            if p_transform_type == TensorTransform.MULTIHEAD_HEADDIM:
                ...
                tensor_transformed = tensor_transformation(
                    tensor_transformed,
                    p_transform_type,
                    num_groups=head_dim,
                    num_heads=num_heads,
                ...
                num_heads = p_transform_config["num_heads"]
                tensor_transformed = tensor_transformation(
                    tensor_transformed, p_transform_type, num_groups=num_heads
                )
