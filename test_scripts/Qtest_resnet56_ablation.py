...
def main(config):
    ...
    output_dir = resolve_output_dir(
        config.output_dir, f"{model_name}_{variant}_{sparsity}"
    )
    ...
    MESSAGING_LOG_FILE = os.path.join(
        log_dir, f"{model_name}_{variant}_{sparsity}_{pruning_start_step}.txt"
    )
    ...
    output_logger.info(
        f"Full average weight bit width for Q{model_name}: {full_average_bit_width} bits"
    )
    ...
    output_logger.info(
        f"Compressed average weight bit width for Q{model_name}: {compressed_average_bit_width} bits"
    )
    ...
    mlflow.log_metric(
        "MAC_reduction", (1.0 - compressed_macs["total"] / full_macs["total"]) * 100
    )
    mlflow.log_metric(
        "BOP_reduction", (1.0 - compressed_bops["total"] / full_bops["total"]) * 100
    )
    mlflow.log_metric(
...
def get_config():
    ...
    parser.add_argument(
        ...
        type=str,
        default="resnet56",
        help="Model architecture to use (resnet20)",
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Initial learning rate for quantization",
    )
    parser.add_argument("--weight_decay", type=float, default=1e-4, help="Weight decay")
    parser.add_argument("--sparsity", type=float, default=0.4, help="Sparsity Level")
    parser.add_argument(
        "--projection_start_step", type=int, default=10, help="Start projection step"
    )
    parser.add_argument(
        "--projection_periods", type=int, default=5, help="Number of projection periods"
    )
    parser.add_argument(
        "--projection_steps", type=int, default=10, help="Number of projection steps"
    )
    parser.add_argument(
        "--pruning_start_step", type=int, default=20, help="Start pruning step"
    )
    parser.add_argument(
        "--pruning_periods", type=int, default=10, help="Number of pruning periods"
    )
    parser.add_argument(
        "--pruning_steps", type=int, default=30, help="Number of pruning steps"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="bit width range minimum (weight quantization)",
    )
    parser.add_argument(
        "--max_bit_wt",
        type=int,
        default=16,
        help="bit width range maximum (weight quantization)",
    )
    parser.add_argument(
        "--min_bit_act",
        type=int,
        default=4,
        help="bit width range minimum (activation quantization)",
    )
    parser.add_argument(
        "--max_bit_act",
        type=int,
        default=6,
        help="bit width range maximum (activation quantization)",
    )
    parser.add_argument("--mix_up", type=int, default=0, help="mixup yes(1) or no(0)")
    parser.add_argument(
        "--label_smooth", type=int, default=0, help="lablel smoothing yes(1) or no(0)"
    )
    parser.add_argument("--seed", type=int, default=0, help="random seed")
    parser.add_argument(
        "--ablation", type=str, default="qhesso", help="ablation experiment name"
    )
