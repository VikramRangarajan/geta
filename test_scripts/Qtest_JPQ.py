...
def main(config):
    ...
    items = list(
        next(iter(bit_dict.values())).keys()
    )  # Get the items from the first category
    bar_width = 0.2
...
def get_config():
    ...
    parser.add_argument(
        "--sparsity_level", type=float, default=0.5, help="Sparsity Level"
    )
    parser.add_argument(
        "--projection_start_step", type=int, default=0, help="Start projection step"
    )
    parser.add_argument(
