        ...
        if btn_config:
            ...
            if channels[0] == 0 or channels[1] == 0:
                ...
                self.residual_function = nn.Sequential(
                    ...
                    nn.ReLU(inplace=True),
                    nn.Conv2d(
                        1, 1, stride=stride, kernel_size=3, padding=1, bias=False
                    ),
                    nn.BatchNorm2d(1),
                    nn.ReLU(inplace=True),
...
class ResNet(nn.Module):
    def __init__(self, block, num_block, num_classes=10, cfg=None):
        ...
        if self.config is None:
            ...
            assert len(num_block) + 1 == len(
                self.config
            )  # first conv + 4 blocks + final linear
            ...
            self.conv2_x = self._make_layer(
                block, self.config[1][-1][-1], num_block[0], 1, layer_id=0
            )
            self.conv3_x = self._make_layer(
                block, self.config[2][-1][-1], num_block[1], 2, layer_id=1
            )
            self.conv4_x = self._make_layer(
                block, self.config[3][-1][-1], num_block[2], 2, layer_id=2
            )
            self.conv5_x = self._make_layer(
                block, self.config[4][-1][-1], num_block[3], 2, layer_id=3
            )
            self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
            ...
            if self.config:
                layers.append(
                    block(
                        ...
                        out_channels,
                        stride,
                        btn_config=self.config[layer_id + 1][idx],
                    )
                )
