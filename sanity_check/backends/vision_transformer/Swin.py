    ...
    def forward(self, x, mask: torch.Tensor | None = None):
        ...
        if self.fused_attn:
            ...
            if mask is not None:
                ...
                mask = mask.view(1, num_win, 1, N, N).expand(
                    B_ // num_win, -1, self.num_heads, -1, -1
                )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(self.num_layers):
            ...
            self.feature_info += [
                dict(
                    num_chs=out_dim, reduction=patch_size * scale, module=f"layers.{i}"
                )
            ]
...
def swin_s3_tiny_224(pretrained=False, **kwargs) -> SwinTransformer:
    ...
    return _create_swin_transformer(
        "swin_s3_tiny_224", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def swin_s3_small_224(pretrained=False, **kwargs) -> SwinTransformer:
    ...
    return _create_swin_transformer(
        "swin_s3_small_224", pretrained=pretrained, **dict(model_args, **kwargs)
    )
...
def swin_s3_base_224(pretrained=False, **kwargs) -> SwinTransformer:
    ...
    return _create_swin_transformer(
        "swin_s3_base_224", pretrained=pretrained, **dict(model_args, **kwargs)
    )
