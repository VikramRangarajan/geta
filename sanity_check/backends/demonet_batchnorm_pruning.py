...
class DemonetBatchnormPruning(nn.Module):
    def __init__(
        ...
        if size == 512:
            self.input_layer = nn.Sequential(
                nn.Conv2d(input_nc, base_nc, kernel_size=7, stride=2, padding=3),
                nn.Conv2d(base_nc, base_nc, kernel_size=7, stride=1, padding=3),
            )
        elif size == 256:
            self.input_layer = nn.Conv2d(
                input_nc, base_nc, kernel_size=7, stride=1, padding=3
            )
        elif size == 64:
            self.input_layer = nn.Conv2d(
                input_nc, base_nc, kernel_size=3, stride=1, padding=1
            )
        else:
        ...
        self.output_layer = nn.Conv2d(
            self.output_nc, self.output_nc, kernel_size=3, stride=1, padding=1
        )
    ...
    def forward(self, x):
        ...
        out = out_list.pop()
        for i in range(self.encoder_layers - self.decoder_layers, self.encoder_layers)[
            ::-1
        ]:
