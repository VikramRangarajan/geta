...
class ResNet_DuBIN(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            3, 64, kernel_size=3, stride=init_stride, padding=1, bias=False
        )
        if ibn_cfg[0] == "b":
        ...
        self.layer1 = self._make_layer(
            block, 64, num_blocks[0], stride=1, ibn=ibn_cfg[0]
        )
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], stride=2, ibn=ibn_cfg[1]
        )
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], stride=2, ibn=ibn_cfg[2]
        )
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, ibn=ibn_cfg[3]
        )
    ...
    def _make_layer(self, block, planes, num_blocks, stride, ibn=None):
        layers = []
        layers.append(
            block(self.in_planes, planes, planes, stride, None if ibn == "b" else ibn)
        )
        ...
        for i in range(1, num_blocks):
            layers.append(
                block(
                    ...
                    planes,
                    1,
                    None if (ibn == "b" and i < num_blocks - 1) else ibn,
                )
            )
...
class Purne_ResNet_DuBIN(nn.Module):
    def __init__(
        ...
        self.conv1 = conv_layer(
            3, self.ch_list[1], kernel_size=3, stride=1, padding=1, bias=False
        )
        if ibn_cfg[0] == "b":
    ...
    def _make_layer(
        self, block, num_blocks, stride, ibn=None, prune_reg="weight", task_mode="prune"
    ):
        layers = []
        layers.append(
            block(
                self.ch_list[self.layer_idx],
                self.ch_list[self.layer_idx + 1],
                self.ch_list[self.layer_idx + 2],
                stride,
